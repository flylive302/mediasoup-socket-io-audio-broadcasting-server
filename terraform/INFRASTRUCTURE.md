...
| Bucket | `flylive-audio-tfstate-<ACCOUNT_ID>` (one bucket, single account — ADR 0028) |
| Keys | `env/production/terraform.tfstate` · `env/staging/terraform.tfstate` (key-per-environment) |
| Region | `ap-south-1` |
| Encryption | SSE-S3 |
| Locking | `use_lockfile = true` (Terraform 1.10+ native, no DynamoDB needed) |

> Bucket and key are never hardcoded in `main.tf` — supply them at init time with
> `-backend-config=backend-production.hcl` or `-backend-config=backend-staging.hcl`
> (generated by `scripts/aws/bootstrap-state.sh`). Staging and production never share a state file.
...
**Fixed:** `main.tf`'s backend `bucket` and `key` are now placeholders that **must** be
overridden via `-backend-config=backend-<env>.hcl` (ADR 0028: single account,
key-per-environment — `env/staging/…` and `env/production/…` in one bucket).
...
both `backend-staging.hcl` and `backend-production.hcl` — so the same script works
in any account. Run once:
```bash
AWS_PROFILE=flylive-prod ./scripts/aws/bootstrap-state.sh
cd terraform && terraform init -reconfigure -backend-config=backend-production.hcl
```
See the cross-cutting runbook `docs/ops/deployment-migration-runbook.md` (root, AWS-era historical) §5.1, §7.5.
...
### 4.5 Point Terraform at the New Bucket

Do **not** edit `main.tf` — bucket and key stay placeholders there.
`scripts/aws/bootstrap-state.sh` writes `backend-production.hcl` and
`backend-staging.hcl`; init with the one for the environment you're working on:
