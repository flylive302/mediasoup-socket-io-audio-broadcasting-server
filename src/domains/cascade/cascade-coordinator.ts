...
export class CascadeCoordinator {
  ...
  async handleCrossRegionJoin(roomId: string): Promise<CascadeJoinResult> {
    ...
    const originBaseUrl = `https://${cascadeInfo.hosting_ip}:${cascadeInfo.hosting_port}`;
    ...
    // Register origin as a remote instance for signaling relay
    const originInstance: RemoteInstance = {
      instanceId: cascadeInfo.hosting_ip,
      baseUrl: originBaseUrl,
    };
    this.cascadeRelay.registerRemote(roomId, originInstance);

    // Register ourselves as an edge in the origin's registry (fire-and-forget)
