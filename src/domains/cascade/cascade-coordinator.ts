...
export class CascadeCoordinator {
  ...
  constructor(
    ...
    this.crossRegionJoin = new CrossRegionJoin(
      roomRegistry,
      laravelClient,
      cascadeRelay,
      this.originSnapshot,
    ...
    this.edgePipeLifecycle = new EdgePipeLifecycle(
      pipeManager,
      roomManager,
      this.crossRegionJoin.originUrls,
      logger,
    );
    this.reversePipeLifecycle = new ReversePipeLifecycle(
      pipeManager,
      this.crossRegionJoin.originUrls,
      logger,
    );
  ...
  isEdgeRoom(roomId: string): boolean {
    return this.crossRegionJoin.isEdgeRoom(roomId);
  }
  ...
  async requestPipeForProducer(
    ...
    return this.edgePipeLifecycle.requestPipeForProducer(
      roomId,
      producerId,
      cluster,
    );
  ...
  async fetchOriginRoomSnapshot(
    ...
    const originBaseUrl = this.crossRegionJoin.getOriginUrl(roomId);
    if (!originBaseUrl) return null;
    return this.originSnapshot.fetchOriginRoomSnapshot(
      originBaseUrl,
      roomId,
      seatCount,
    );
  ...
  async handleRemoteProducerClosed(
    roomId: string,
    producerId: string,
  ): Promise<string | null> {
    if (!this.isEdgeRoom(roomId)) return null;
    return this.edgePipeLifecycle.handleRemoteProducerClosed(
      roomId,
      producerId,
    );
  }

  async handleRemoteNewProducer(
    roomId: string,
    producerId: string,
  ): Promise<string | null> {
    if (!this.isEdgeRoom(roomId)) return null;
  ...
  async handleOriginClosed(roomId: string): Promise<void> {
    ...
    this.logger.info(
      { roomId },
      "CascadeCoordinator: origin closed, tearing down edge",
    );
    await this.cleanup(roomId);
    // Evict the edge's mediasoup cluster from RoomManager. cleanup() above only
    // tears down cascade plumbing (pipes/relay/cross-region maps); without this
    // the RoomMediaCluster lingers in roomManager.rooms as a ghost and a later
    // join routed here is served on an orphaned island (same-region split-brain).
    // evictLocalRoom is local-only — it must NOT release the origin's CAS key.
    await this.roomManager.evictLocalRoom(roomId);
  }
  ...
  async setupReversePipe(
    ...
    return this.reversePipeLifecycle.setupReversePipe(
      roomId,
      edgeProducer,
      cluster,
      userId,
  ...
  async cleanup(roomId: string): Promise<void> {
    ...
    this.logger.debug(
      { roomId },
      "CascadeCoordinator: room cascade cleanup complete",
    );
  }
}
