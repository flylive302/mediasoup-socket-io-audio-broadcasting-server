...
export const mediaHandler = (
  ...
  socket.on('transport:create', async (rawPayload: unknown, callback) => {
    ...
    if (!payloadResult.success) {
        if (callback) callback({ error: 'Invalid payload', details: payloadResult.error.format() });
        return;
    }
    ...
    try {
        const transport = await routerMgr.createWebRtcTransport(type === 'producer');
  ...
  socket.on('audio:consume', async (rawPayload: unknown, callback) => {
    ...
    const { roomId, transportId, producerId, rtpCapabilities } = payloadResult.data;
