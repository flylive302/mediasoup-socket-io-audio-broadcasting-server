...
import type { BatchProcessingResult, GiftTransaction, RoomStatusUpdate } from './types.js';
...
export class LaravelClient {
   ...
   */
  async processGiftBatch(transactions: GiftTransaction[]): Promise<BatchProcessingResult> {
    // Strip internal socket ID before sending
    const payload = transactions.map(({ sender_socket_id, ...rest }) => rest);
    ...
    result.failed = result.failed.map(fail => {
      const original = transactions.find(t => t.transaction_id === fail.transaction_id);
   ...
   */
  async updateRoomStatus(roomId: string, status: RoomStatusUpdate): Promise<void> {
    try {
      const response = await this.post(`/api/v1/internal/rooms/${roomId}/status`, status);
      
      if (!response.ok) {
        this.logger.error({ status: response.status, roomId }, 'Failed to update room status');
      }
    } catch (error) {
