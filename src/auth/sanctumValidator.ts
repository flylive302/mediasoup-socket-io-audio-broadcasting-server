...
export class SanctumValidator {
  ...
  async validate(token: string): Promise<AuthenticatedUser | null> {
    ...
    try {
        ...
        if (isRevoked) {
            this.logger.warn('Attempted use of revoked token (validated via Redis)');
    ...
    try {
      ...
      try {
        response = await fetch(`${config.LARAVEL_API_URL}/api/v1/internal/auth/validate`, {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        this.logger.warn({ status: response.status }, 'Laravel token validation failed');
        return null;
      }
